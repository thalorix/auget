...
sys.path.insert(0, os.path.join(BASE, "..", "auget4"))
...
<div class="nav">
  ...
  <div>
    {% if current_user.is_authenticated %}
      ...
      <a href="{{ url_for('analyze_page') }}">Analizza</a>
      <a href="{{ url_for('pricing') }}">Piani</a>
      <a href="{{ url_for('logout') }}">Esci</a>
    {% else %}
      <a href="{{ url_for('login') }}">Accedi</a>
...
def index():
    ...
    {% if not current_user.is_authenticated %}
    <p><a href="{{ url_for('register') }}" style="color:#f0b429">Registrati</a> per iniziare.</p>
    {% endif %}
    </div>"""
...
def register():
    ...
    <p style="text-align:center;margin-top:1rem">Hai gia un account? <a href="{{ url_for('login') }}" style="color:#f0b429">Accedi</a></p>
    </div>"""
...
def login():
    ...
    <p style="text-align:center;margin-top:1rem">Demo: <strong>demo@demo.com</strong> / <strong>demo123</strong></p>
    </div>"""
...
def pricing():
    ...
    <div class="tier {% if current_user.subscription_tier == 'basic' %}active{% endif %}">
      ...
      <form method="post" action="{{ url_for('checkout', tier='basic') }}">
    ...
    <div class="tier {% if current_user.subscription_tier == 'pro' %}active{% endif %}">
      ...
      <form method="post" action="{{ url_for('checkout', tier='pro') }}">
    ...
    <div class="tier {% if current_user.subscription_tier == 'enterprise' %}active{% endif %}">
      ...
      <form method="post" action="{{ url_for('checkout', tier='enterprise') }}">
...
def analyze_page():
    ...
    <form method="post" enctype="multipart/form-data" action="{{ url_for('do_analyze') }}">
...
if __name__ == "__main__":
    print("Buffett Analyzer WEB con login: http://127.0.0.1:5000")
    print("Account demo: demo@demo.com / demo123")
    app.run(host="0.0.0.0", port=5000, debug=False)
'''
with open("app.py", "w", encoding="utf-8") as f:
    f.write(CODE)
print("OK: app.py generato, byte:", len(CODE))
